...
/* 
    This will teach you how imperious works.

	Please credit imperious & me (speedsterkawaii) for updating?
*/

const char* ExploitName = "Transpiler"; // The exploit name (changeable).
#define PipeName "\\\\.\\pipe\\Transpiler" // Only change "Transpiler".
...
bool ExecuteScript(DWORD RS, lua_State* L, std::string Script) {
	...
	if (luaL_loadbuffer(L, Script.c_str(), Script.size(), "@Transpiler"))
	{
...
void Thread() {
	ConsoleBypass("Imperious Transpiler // Speed Softwares // SpeedSterKawaii");
